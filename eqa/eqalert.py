...
def main():
    ...
    process_keys = threading.Thread(target=eqa_keys.process,
                                    args=(keyboard_q, system_q, display_q,
                                          sound_q, exit_flag, heal_parse, spell_parse,
                                          raid, state.chars))
    ...
    process_action = threading.Thread(target=eqa_action.process,
                                      args=(action_q, system_q, display_q,
                                            sound_q, heal_q, damage_q, exit_flag,
                                            heal_parse, spell_parse, raid, cfg_reload,
                                            config, base_path))
            ...
            if not system_q.empty():
                ...
                if new_message.type == "system":
                    ...
                    # Update character
                        ...
                        new_char_log = config["settings"]["paths"]["char_log"] + \
                        ...
                        if os.path.exists(new_char_log):
                            ...
                            log_watch.add_watch(char_log,
                                                pyinotify.IN_CLOSE_WRITE, callback)
                    ...
                    # Reload config
                        ...
                        process_action = threading.Thread(target=eqa_action.process,
                                                          args=(action_q, system_q,
                                                                display_q, sound_q, heal_q,
                                                                damage_q, exit_flag, heal_parse,
                                                                spell_parse, raid, cfg_reload,
                                                                config, base_path))
                        process_action.daemon = True
                        process_action.start()
                        process_sound = threading.Thread(target=eqa_sound.process,
                                                         args=(config, sound_q,
                                                               exit_flag, cfg_reload))
                        process_sound.daemon = True
                        process_sound.start()
